...
let of_string : string -> t option =
 fun s ->
  if String.length s <= max_length_int then
    Some s
  else
    None
...
let length : t -> Uint63.t = fun s -> Uint63.of_int (String.length s)

(* Out of bound access gives '\x00'. *)
let get : t -> Uint63.t -> char63 =
 fun s i ->
  let i = Uint63.to_int_min i max_length_int in
...
let compile : t -> string = Printf.sprintf "Pstring.unsafe_of_string %S"
