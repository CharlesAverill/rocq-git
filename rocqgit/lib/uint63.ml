...
let of_int i = i [@@ocaml.inline always]
...
let hash i = i [@@ocaml.inline always]
...
let l_or x y = x lor y [@@ocaml.inline always]

let l_xor x y = x lxor y [@@ocaml.inline always]

(* addition of int63 *)
let add x y = x + y [@@ocaml.inline always]

(* subtraction *)
let sub x y = x - y [@@ocaml.inline always]

(* multiplication *)
let mul x y = x * y [@@ocaml.inline always]
...
let addmuldiv p x y = l_or (l_sl x p) (l_sr y (uint_size - p))
...
(* signed comparison *)
let lts (x : int) (y : int) = x < y [@@ocaml.inline always]

let les (x : int) (y : int) = x <= y [@@ocaml.inline always]
...
let div21 xh xl y =
  ...
  for _i = 0 to 62 do
    ...
    nl := !nl lsl 1 ;
    q := !q lsl 1 ;
    if Int64.unsigned_compare !nh y >= 0 then begin
      q := !q lor 1 ;
      nh := Int64.sub !nh y
    end
  done ;
...
let equal (x : int) (y : int) = x = y [@@ocaml.inline always]
...
let compares (x : int) (y : int) = Int.compare x y
...
let head0 x =
  let r = ref 0 in
  let x = ref x in
  if !x land 0x7FFFFFFF00000000 = 0 then
    r := !r + 31
  else
    x := !x lsr 31 ;
  if !x land 0xFFFF0000 = 0 then (
    x := !x lsl 16 ;
    r := !r + 16
  ) ;
  if !x land 0xFF000000 = 0 then (
    x := !x lsl 8 ;
    r := !r + 8
  ) ;
  if !x land 0xF0000000 = 0 then (
    x := !x lsl 4 ;
    r := !r + 4
  ) ;
  if !x land 0xC0000000 = 0 then (
    x := !x lsl 2 ;
    r := !r + 2
  ) ;
  if !x land 0x80000000 = 0 then (
    x := !x lsl 1 ;
    r := !r + 1
  ) ;
...
let tail0 x =
  ...
  if !x land 0xF = 0 then (
    x := !x lsr 4 ;
    r := !r + 4
  ) ;
  if !x land 0x3 = 0 then (
    x := !x lsr 2 ;
    r := !r + 2
  ) ;
...
let is_uint63 t = Obj.is_int t [@@ocaml.inline always]
